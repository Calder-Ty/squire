...
pub const ParserCallback = *const fn (vtparse: *VTParser, action: pt.Action, char: u8) void;
...
pub const VTParser = struct {
    ...
    pub fn parse(self: *VTParser, data: []const u8, len: usize) void {
        ...
        while (i < len) : (i += 1) {
            const ch = data[i];
            const state_change = pt.parse_table[@intFromEnum(self.state) - 1][ch];
    ...
    fn doAction(self: *VTParser, in_action: pt.Action, ch: u8) void {
        ...
        switch (in_action) {
            pt.Action.PRINT, pt.Action.EXECUTE, pt.Action.HOOK, pt.Action.PUT, pt.Action.OSC_START, pt.Action.OSC_PUT, pt.Action.OSC_END, pt.Action.UNHOOK, pt.Action.CSI_DISPATCH, pt.Action.ESC_DISPATCH => self.cb(self, in_action, ch),

            pt.Action.IGNORE => {},

            pt.Action.COLLECT => {
                // Append the character to the intermediate params
                if (self.num_intermediate_chars + 1 > MAX_INTERMEDIATE_CHARS) {
                    // Is this a bool?
                    self.ignore_flagged = 1;
                } else {
                    self.intermediate_chars[self.num_intermediate_chars] = ch;
            ...
            pt.Action.CLEAR => {
                ...
                self.num_params = 0;
                self.ignore_flagged = 0;
            },
        }
    }

    fn doStateChange(self: *VTParser, change: pt.ParserState, ch: u8) void {
        ...
        if (new_state) |ns| {
            ...
            const exit_action = pt.exit_actions[@intFromEnum(self.state) - 1];
            const entry_action = pt.entry_actions[@intFromEnum(ns) - 1];

            if (exit_action) |act| {
                self.doAction(self, act, 0);
            }
            if (in_action) |act| {
                self.doAction(self, act, ch);
            }
            if (entry_action) |act| {
                self.doAction(self, act, 0);
            }
...
test "VTParser" {
    ...
    fn parserCallback(vtparse: *VTParser, action: pt.Action, char: u8) void {
        std.debug.assert(input[i] == char);
        i++;
    }

    parser = VTParser.init(parserCallback);
